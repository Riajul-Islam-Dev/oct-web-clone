...
import { officeLocationsData } from "@/utils/helperData";
...
export default function Page() {
  ...
  // Pagination
  const totalPages = Math.ceil(officeLocationsData.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const paginatedData = officeLocationsData.slice(
    startIndex,
    ...
    <Sheet>
      <div className="px-4 py-4 flex-1">
        <div className="flex justify-between items-center mb-4">
          <h4 className="text-lg font-ubuntu">Office Locations Management</h4>
        ...
        <table className="w-full table-auto border-separate border-spacing-y-1 mb-12 font-ubuntu text-sm">
          <thead className="bg-white shadow-[0px_4px_30px_0px_#0000000F] rounded mb-2">
            <tr className="px-4">
              <td className="px-4 py-1.5 text-nowrap text-[#2A2B2F]">
                Office Name
              </td>
              <td className="px-4 py-1.5 text-nowrap text-[#2A2B2F]">
                Address
              ...
              <td className="px-4 py-1.5 text-nowrap text-[#2A2B2F] text-center">
                Quick Actions
              </td>
            </tr>
          </thead>
          <tbody className="mt-4">
            {paginatedData.map((row) => (
              ...
              >
                <td className="px-4 py-1.5 text-[#5E6369]">{row.officeName}</td>
                <td className="px-4 py-3 text-[#5E6369]">{row.address}</td>
