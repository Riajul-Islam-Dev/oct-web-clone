...
interface CabinClassData {
  policyLevel: string;
  travelerType: string;
  class: string;
}

interface CabinClassTableProps {
  data: CabinClassData[];
}

export function CabinClassTable({ data }: CabinClassTableProps) {
  return (
      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-[#f7f9fc] border-b border-[#e2e8f0]">
            ...
            <TableHead className="text-[#5e6369] font-medium text-sm py-3 px-4">Class</TableHead>
        ...
        <TableBody>
          {data.map((row, index) => (
            <TableRow key={index} className="border-b border-[#e2e8f0] hover:bg-[#f5fafd]">
              ...
              <TableCell className="py-3 px-4 text-sm text-[#18181a]">{row.class}</TableCell>
